...
function novoJogo() {
    ...
    for (let i = 0; i < 200; i++) {
        document.getElementById('palavras').innerHTML += formatarPalavra(palavraAleatoria());
    }
...
document.getElementById('jogo').addEventListener('keyup', ev => {
    ...
    if (isLetra) {
        ...
        if (key === esperada) {
            ...
            const proximaLetra = currentLetra.nextSibling;

            if (proximaLetra && proximaLetra.classList.contains("letra")) {
                adicionarClasse(proximaLetra, "current");
            } else {
                adicionarClasse(currentLetra, "current");
            }
    ...
    if (isEspaco) {
        ...
        adicionarClasse(proximaPalavra, "current");

        removerClasse(currentLetra, "current");
        const primeiraLetra = proximaPalavra.querySelector('.letra');
        adicionarClasse(primeiraLetra, "current");
